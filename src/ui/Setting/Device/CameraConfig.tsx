      ...
      <Listbox value={selected.label} onChange={handleChange}>
        <div className="relative mt-1">
          <Listbox.Button className="dark:bg-gray-600 relative w-full cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left shadow-md focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 sm:text-sm">
            <span className=" dark:bg-gray-600 block truncate">{selected?.label}</span>
          ...
          <Transition
            ...
            leaveTo="opacity-0"
          >
            <Listbox.Options className="dark:bg-gray-600  absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              {devices.map((device, index) => (
                ...
                >
                  {({ selected }) => (
                    <>
                      <span
                        className={`dark:text-white block truncate ${selected ? "font-medium" : "font-normal"}`}
                      >
                        {device.label}
                      </span>
                      {selected ? (
